...
from model_improve import DM2FNet_woPhy_new, DM2FNet_attention_in, DM2FNet_new2, ours_wo_AFIM, DM2FNet_attention_chuan
from datasets import SotsDataset, OHazeDataset, SotsDataset_mix, HazeRDDataset
...
from MixDehazeNet.datasets.loader import PairLoader_its, PairLoader_sots, PairLoader_hazerd
from MixDehazeNet.utils import AverageMeter, write_img, chw_to_hwc
import cv2
...
# exp_name = 'RESIDE_ITS_DM2FNet_woPhy_new'
exp_name = 'RESIDE_ITS_mixDehazenet'
...
args = {
    ...
    'snapshot': 'iter_100_loss_0.02335_lr_0.000002',
...
def main():
    with torch.no_grad():
        ...
        for name, root in to_test.items():
            if 'SOTS' in name:
                net = MixDehazeNet_s().cuda()
                dataset = PairLoader_sots(root, 'test', 'valid', 
							  256)
                ...
                net = MixDehazeNet_s().cuda()
                dataset = PairLoader_hazerd(root)
            else:
            ...
            for idx, data in enumerate(dataloader):
                ...
                for r, f in zip(res.cpu(), fs):
                    r = r * 0.5 + 0.5
                    out_img = chw_to_hwc(r.detach().cpu().squeeze(0).numpy())
                    write_img(os.path.join(ckpt_path, exp_name,
                                     '(%s) %s_%s' % (exp_name, name, args['snapshot']), '%s.png' % f), out_img)
                    # to_pil(r).save(
